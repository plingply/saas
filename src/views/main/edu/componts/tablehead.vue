<template>
  <el-dialog title="设置表头" :visible.sync="dialogVisible" width="550px">
    ...
    <span slot="footer" class="dialog-footer">
      ...
      <el-button type="primary" @click="setTablehead" :loading="loading">确定</el-button>
    </span>
  </el-dialog>
</template>

<script>
import tablehead from "@/util/clur_table_head";

let thead = []

for(let key in tablehead){
    if(tablehead[key].checked){
        thead.push(tablehead[key].value)
    }
}
...
export default {
  ...
  methods: {
    ...
    setTablehead() {
      ...
      this.$ga.event("线索管理SaaS端", "设置表头-设置", "merchant_id", this.mymange);
